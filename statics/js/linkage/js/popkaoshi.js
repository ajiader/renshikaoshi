function open_linkage(id,name,container,linkageid) {
	...
	var content = '<div class="linkage-menu"><h6><a href="javascript:;"  onclick="get_parent(this,0)" class="rt"><<返回主菜单</a><span>'+name+'</span> <a href="javascript:;" onclick="get_child(\'0\',\'清除选择\',\'0\')">取消地区</a> <a href="javascript:;" onclick="get_parent(this)" id="parent_'+id+'" parentid="0"><img src="statics/images/icon/old-edit-redo.png" width="16" height="16" alt="返回上一级" /></a></h6><div class="ib-a menu" id="ul_'+id+'">';
...
function get_child(nodeid,nodename,parentid) {
	...
	var url = "kapi.php?op=get_linkage&act=ajax_getlist&parentid="+nodeid+"&keyid="+returnkeyid;
	...
	$.getJSON(url+'&callback=?',function(data){
		...
		if(data) {
			...
			content =  '<a href="javascript:;" onclick=\'set_val("'+nodename+'",'+nodeid+');  art.dialog({id:"edit_'+returnid+'"}).close(); \' style="color:red;font-weight:bold">'+nodename+'</a>'; 
...
function get_parent(obj,parentid) {
	...
	var url = "kapi.php?op=get_linkage&act=ajax_getlist&linkageid="+linkageid+"&keyid="+returnkeyid;
...
function get_path(nodeid,returnid) {
	...
	var url = "kapi.php?op=get_linkage&act=ajax_getpath&parentid="+nodeid+"&keyid="+returnkeyid;
...
function set_val(nodename,nodeid) {
	...
	var url = "kapi.php?op=get_linkage&act=ajax_gettopparent&linkageid="+nodeid+"&keyid="+returnkeyid;
