...
(function () {
    ...
    scrawl.prototype = {
        ...
        originalState:function (options) {
            ...
            me.ext = options.ext;//同步画笔颜色

            $G("fileForm").action = editor.options.scrawlUrl + "&action=tmpImg" + me.ext;//初始form提交地址
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if(!!base64){
            ajax.request(editor.options.scrawlUrl+settings.ext, {
                timeout:100000,
                content:base64,
                onsuccess:function (xhr) {
                    if (!scrawlObj.isCancelScrawl) {
                        ...
                        if (responseObj.state == "SUCCESS") {
                            ...
                            $.get('index.php?m=attachment&c=ueditor&a=swfupload_json&aid='+responseObj.aid+'&src='+responseObj.url+'&filename='+responseObj.filename);
                            dialog.close();
                        } else {
