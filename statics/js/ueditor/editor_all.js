...
//                            }
            ...
            document.getElementById('paginationtype').value=2;
...
//                }
                var re=/\[page\](.+)\[\/page\]/;
                if ( me.options.pageBreakTag && node.type == 'text' ) {                    
                    ...
                    else if ( node.data.replace( /\s/g, '' ) == me.options.pageBreakTag ) {
                        node.type = 'element';
                        node.name = node.tag = 'hr';
        ...
        transformOutput: function ( node ) {
            function visitNode( node ) {
                ...
                if ( node.tag == 'hr' && node.attributes['class'] == 'subtitle' ) {
                    delete node.tag;
                    node.type = 'text';
                    node.data = '[page]'+node.attributes['subtitle']+'[/page]';
                    delete node.children;
...
UE.plugins['catchremoteimage'] = function () {
    ...
    function catchremoteimage(imgs, callbacks) {
        ...
        ajax.request( me.options.catcherUrl, tmpOption);       
    }
            ...
            catchremoteimage(remoteImages, {
                ...
                success:function (xhr) {
                    ...
                    var srcUrls = info.srcUrl.split(separater),
                        ...
                        aids = info.aids.split(separater);
                    for (var i = 0, ci; ci = imgs[i++];) {
                        var src = ci.getAttribute("data_ue_src") || ci.src || "";
                        for (var j = 0, cj; cj = srcUrls[j++];) {
                            ...
                            if (src == cj && url != "error") {  //抓取失败时不做替换处理
                                ...
                                $.get('index.php?m=attachment&c=ueditor&a=swfupload_json&aid='+aid+'&src='+newSrc+'&filename='+newSrc);
                                break;
                            }
                        }
                    }
...
(function () {
    ...
    var dialogBtns = {
        ...
        ok:['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage',
            'insertvideo', 'highlightcode', 'insertframe', 'edittd', 'scrawl', 'template','background','subtitle']
