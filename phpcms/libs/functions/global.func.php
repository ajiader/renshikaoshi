...
* $timeout 过期时间
*/
function mem_page($flag,$timeout='18000')
{
	...
	//echo strstr($uri,'php');
	        if(!strstr($uri,'php')){
			    setcache($uri, $htmlcontent, '', 'memcache', 'template', $timeout); // 写入缓存 1小时后失效
			}
		}
...
function getMemcacheKeys() {
	...
	$host = $cacheconfig['template']['hostname'];
	$port = $cacheconfig['template']['port'];
    $mem=new Memcache();
...
function pageurl($urlrule, $page, $array = array()) {
	if(strpos($urlrule, '~')) {
        ...
        if($GLOBALS['URL_ARRAY']['pinyin']){
        	$urlrule = $urlrules[2];
        }elseif($GLOBALS['URL_ARRAY']['typeid']){
        	$urlrule = $urlrules[3];
        }else{
...
function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0) {
	...
	if($datas['style']=='1') {
		...
		if(!defined('LINKAGE_INIT_1')) {
			...
			$string .= '<script type="text/javascript" src="'.JS_PATH.'linkage/js/popkaoshi.js"></script>';
		}
		...
		$string .= '<script type="text/javascript">
					$(function(){
						...
						$ld5.ld({ajaxOptions : {"url" : "'.APP_PATH.'kapi.php?op=get_linkage&act=ajax_select&keyid='.$linkageid.'"},defaultParentId : 0,style : {"width" : 120}})	 
 ...
 * @param  $keyid 菜单keyid
 * @param  $space 菜单间隔符
 * @param  $tyoe 1 返回间隔符链接，完整路径名称 3 返回完整路径数组，2返回当前联动菜单名称，4 直接返回ID 5 返回链接
		...
		if(array_key_exists($linkageid,$infos)) {
			...
			if(count($result)>0) {
                ...
                if($type ==5){
				    foreach($result as $v){
						if($v[pinyin]){
							$siteurl = siteurl(intval($_GET[siteid]));
						  $results .= "$space <a href=\"{$siteurl}/{$v[pinyin]}/\">{$v[name]}</a>"; 
						}
					}
					$result = $results;
				}
...
function getidurl($catid,$id,$modelid)
{
	$cid=intval($catid);
	$db=pc_base::load_model('content_model');
	$db->table_name=$db->db_tablepre.'model';
	$r=$db->get_one("`modelid`={$modelid}");
	$db->table_name=$db->db_tablepre.$r['tablename'];
	$n=$db->get_one("`id`={$id}");
	$array=array();
	$array['url']=$n['url'];
	$array['title']=$n['title'];
	...
	$db->table_name=$db->db_tablepre."category";
	$c=$db->get_one("`catid`={$catid}");
	$array['catname']=$c['catname'];
	$sid=$c['siteid'];
	$db->table_name=$db->db_tablepre.'site';
...
function string2img($txt, $fonttype = 5, $fontsize = 16, $font = '', $fontcolor = 'FF0000',$transparent = '1') {
	...
	if(function_exists("imagepng")) {
		...
		$txt = '<img src="'.APP_PATH.'kapi.php?op=creatimg&txt='.$txt.'&fonttype='.$fonttype.'&fontsize='.$fontsize.'&font='.$font.'&fontcolor='.$fontcolor.'&transparent='.$transparent.'" align="absmiddle">';
	}
	return $txt;
}
 ...
 */
 ...
 //采集的栏目id
function GetCaijiCatid()
{
	return array('281','282','283','284','307');
}


function xp_get_linkage($keyid)
{

	$city_db = pc_base::load_model('linkage_model');
	$city_db->get_one(array('linkageid'=>$keyid));
     // $datas = getcache(1,'linkage');
     // $infos = $datas['data'][$keyid];
	$infos = $city_db->get_one(array('linkageid'=>$keyid));
	$parentid=$infos['parentid'];
	if($parentid==0){
		$linkageid=$infos['linkageid'];
		return $linkageid;
	}else{
		return xp_get_linkage($infos['parentid']);
	}
	
}

//批量过滤
function replace_arr($str,$array=array('您可以拨打联系电话：028-66820000，','联系电话：028-66820000，','咨询电话：028-66820000（王老师），', '或拨打电话028-66820000！'), $restr="")
{
	foreach ($array as $key => $value) {
		$str = str_replace($value, $restr, $str);
	}
	return $str;
...
//分词查询替换
function replace_search($findarr,$str)
{
	if (is_array($findarr)) {
		foreach ($findarr as $key => $value) {
			$value = trim($value);
			if (strlen($value) > 3) {
				$str = str_replace($value, '<font color="red">'.$value.'</font>', $str);
			}
		}
		return $str;
	}else{
		$str = str_replace($findarr, '<font color="red">'.$findarr.'</font>', $str);
	}
	return $str;
