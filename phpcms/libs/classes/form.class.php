...
class form {
     ...
     * @param string $style 是否禁用分页和子标题
     */
    public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1, $alowuploadexts = '', $height = 200, $disabled_page = 0, $allowuploadnum = '10', $style=1) {
        if ( $style == 1) {
            $p = "/^[a-zA-Z]+:\/\/(.+?)\//i";
            ...
            $whoUseMe= $_GET["m"];
            if($whoUseMe=="member"||$whoUseMe=="yp"||$admin_url==""){
                $ueditor_path =  JS_PATH. 'ueditor/';
            }else{
                $ADMIN_URL=str_replace($localDomain,$admin_url,APP_PATH);
                $ueditor_path= str_replace($localDomain,$admin_url,JS_PATH).'ueditor/';
            }       
            ...
            //$opt[] = "imagePath:'" . $upload_url."'";
            //$opt[] = "filePath:'" . $upload_url."'";
            //$opt[] = "catcherPath:'" . $upload_url."'";
            //$opt[] = "imageManagerPath:'" . $upload_url."'";
            //$opt[] = "wordImagePath:'" . $upload_url."'"; 
            //$opt[] = "scrawlPath:'" . $upload_url."'";
            if ($allowupload) {   
                $authkey = upload_key("$allowuploadnum,$alowuploadexts,$allowbrowser");
                $iframeUrlMap="'insertimage':'". $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=imagespload&authkey={$authkey}&module={$module}&catid={$catid}&args={$allowuploadnum},{$alowuploadexts},{$allowbrowser}',";
                $iframeUrlMap .="'wordimage':'". $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=wordimage&authkey={$authkey}&module={$module}&catid={$catid}&args={$allowuploadnum},{$alowuploadexts},{$allowbrowser}',";
                $iframeUrlMap .="'attachment':'". $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=swfupload&authkey={$authkey}&module={$module}&catid={$catid}&args={$allowuploadnum},{$alowuploadexts},{$allowbrowser}',";
                $iframeUrlMap .="'scrawl':'". $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=scrawl&authkey={$authkey}&module={$module}&catid={$catid}&args={$allowuploadnum},{$alowuploadexts},{$allowbrowser}'";            
                $opt[] = "iframeUrlMap:{" . $iframeUrlMap."}";            
                $opt[] = "catcherUrl:'" . $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=imagescatch&authkey={$authkey}&module={$module}&catid={$catid}&args={$allowuploadnum},{$alowuploadexts},{$allowbrowser}'";
                $opt[] = "localDomain:['127.0.0.1','localhost','img.baidu.com','".$localDomain."']";
                $opt[] = "scrawlUrl:'" . $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=scrawlUp'";
                $opt[] = "imageManagerUrl:'" . $ADMIN_URL . "goshouye.php?m=attachment&c=ueditor&a=imageslist&authkey={$authkey}&module={$module}&catid={$catid}&args={$allowuploadnum},{$alowuploadexts},{$allowbrowser}'";            
            }
            $str .= "<script type=\"text/javascript\">\r\n";
            ...
            if($allowupload) $str .= "filebrowserUploadUrl : '".APP_PATH."goshouye.php?m=attachment&c=attachments&a=upload&module=".$module."&catid=".$catid."&dosubmit=1',\r\n";
            if($color) {
    ...
    public static function images($name, $id = '', $value = '', $moudle = '', $catid = '', $size = 50, $class = '', $ext = '', $alowexts = '', $thumb_setting = array(), $watermark_setting = 0) {
        ...
        if (!defined('IMAGES_INIT')) {
            $str = '<script type="text/javascript" src="' . JS_PATH . 'swfupload/swfcuppic.js"></script>';
            define('IMAGES_INIT', 1);
        }
    ...
    public static function checkcode($id = 'checkcode', $code_len = 4, $font_size = 20, $width = 130, $height = 50, $font = '', $font_color = '', $background = '') {
        return "<img id='$id' alt='看不清楚？点击换一组验证码。' title='看不清楚？点击换一组验证码。' onclick='this.src=this.src+\"&\"+Math.random()' src='" . SITE_PROTOCOL . SITE_URL . WEB_PATH . "kapi.php?op=checkcode&code_len=$code_len&font_size=$font_size&width=$width&height=$height&font_color=" . urlencode($font_color) . "&background=" . urlencode($background) . "'>";
    }
