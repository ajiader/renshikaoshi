...
class content_tag {
			...
			if(isset($data['where'])) {
				...
				if($this->category[$catid]['child']) {
					$catids_str = $this->category[$catid]['arrchildid'];
					 //排除采集
	                $catids_str = str_replace('284,','',$catids_str); //致睿采集
	                $catids_str = str_replace('282,','',$catids_str); //英语采集
					$catids_str = str_replace(',283','',$catids_str); //计算机采集
	...
	public function lists($data) {
		...
		if(isset($data['where'])) {
			...
			if($this->category[$catid]['child']) {
				...
				$pos = strpos($catids_str,',')+1;
				$catids_str = substr($catids_str, $pos);
				// $sql = "islink=0 and status=99 AND catid IN ($catids_str)".$thumb;
	...
	public function position($data) {
		...
		//获取地区id
		$cityid = ($data['cityid']);
		if($cityid) $sql .= "`cityid` in ($cityid) AND ";
		...
		if(empty($pos_arr) && $cityid){
			$sql = str_replace("`cityid` in ($cityid) AND ", '', $sql); //如果没有数据就去掉地方属性
			$pos_arr = $this->position->select($sql, '*', $data['limit'],'rand()');
		}
		...
		if(!empty($pos_arr)) {
			foreach ($pos_arr as $info) {
				...
				$list = getidurl($info['catid'],$info['id'],$info['modelid']);
				
				$array[$key]['url'] = $list['url'];
