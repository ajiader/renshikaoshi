...
class index {
	...
	public function init() {
		...
		$index_url =  siteurl($siteid).$_SERVER['REQUEST_URI'] ;
		...
		$CATEGORYS2 = getcache('category_content_2','commons');
		$template = ($siteid == 1)?'index':'index_'.$siteid ;
        //$template = ($siteid == 1)?'index':'category_gwy';
		//
		//$htmlcontent = getcache($uri,'','memcache','template');
		ob_start();
	...
	public function show() {
		...
		if(!$catid || !$id) {page404(); /*showmessage(L('information_does_not_exist'),'blank');*/}
		$_userid = $this->_userid;
		$_username = $this->_username;
		$_groupid = $this->_groupid;
		...
		if(!$r){page404(); /* showmessage(L('info_does_not_exists'),'blank');*/}
		...
		if ($rs['city_id']>0) {
			$cityList = $this->city_db->get_one(array('linkageid'=>$rs['city_id']));
			$topparentid = xp_get_linkage($rs['city_id']);
			if ($topparentid > 0) {
		...
		$SEO = seo($siteid, '', $title, $description, $seo_keywords);
		...
		//加载公务员随机话术
		if ($siteid==2) {
			$randtext = getcache('showtxt_2','content');
		}elseif ( strstr($CATEGORYS[$catid]['arrparentid'],'0,10') or $catid == 10 ) {
			$randtext = getcache('showtxt_jsj','content');
		}elseif ( strstr($CATEGORYS[$catid]['arrparentid'],'0,9') or $catid == 9 ) {
			$randtext = getcache('showtxt_en','content');
		}elseif ( strstr($CATEGORYS[$catid]['arrparentid'],'0,11') or $catid == 11 ) {
			//$randtext = getcache('showtxt_zyzg','content');
		}
		// print_r($randtext);exit;
	    if(is_array($randtext))
	         $randcontent = $randtext[array_rand($randtext)];
	...
	protected function _category_priv($catid) {
	 ...
	 /**  
         ...
         * @param <type> $catdir  
         */  
        function _getCategoryId($catdir,$categorydir){   
            if($catdir==""){
			    $catdir = $categorydir;
				$categorydir = "";
				if(strstr($catdir,'/')){
				   $catdir = substr($catdir,0,-1); 
				}
			}
			
			/**$arr = explode('/',$catdir);
			$catdir =  $arr[count($arr)-1];
			$parentdir = str_replace($catdir,'',$old_catdir);
			**/
			
            $this->category_db = pc_base::load_model('category_model');   
            $result = $this->category_db->select(array('catdir'=>$catdir,'parentdir'=>$categorydir));   
            return $result;   
         }
	...
	*/
	function zhuanjia()
	{
		...
		$urlrules = getcache('urlrules','commons');    
		$urlrule = $urlrules[35];//调用url规则 
        $zhuanjiaurl = str_replace('{$page}','1',$urlrule);
		$urlrule = str_replace('{$id}',$id,$urlrule);
		$zj_db = pc_base::load_model('zhuanjia_model');
		$zj_list = $zj_db->listinfo("siteid=$siteid");
		$zjarr = array();

		foreach($zj_list as $v)
		{
			$v['url'] = siteurl($siteid).str_replace('{$id}',$v['id'],$zhuanjiaurl);
			$zjarr[$v['id']] = $v;
			
		}
		if($zjarr[$id]['id'] <= 0) {showmessage(L('information_does_not_exist'),'blank');}
		$name = $zjarr[$id]['name']; //获取专家老师姓名

		$MODEL = getcache('model','commons');
        $tablename = $this->db->table_name = $this->db->db_tablepre.$MODEL[$modelid]['tablename'];
		
		//seo
		$SEO = seo($siteid,'',$name.'-专家文章');
		...
		$tabledataname = $this->db->table_name = $tablename.'_data';

		include template('content','list_zhuanjia');
	}
	...
	function test()
	{
		...
		if($siteid == 1){
		    $contents = ' This is first 1';
	  	}else{
		   	 $contents = ' This is first 2';
		}
		$uri =  "/";
		echo $uri;
		    //setcache($uri, $contents, '', 'memcache', 'template', 60);

			echo getcache($uri,'','memcache','template');
	}
}
?>