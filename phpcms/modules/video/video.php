...
class video extends admin {
	...
	public function __construct() {
		...
		if(empty($this->setting) && ROUTE_A!='setting') {
			showmessage(L('video_setting_not_succfull'), 'goshouye.php?m=video&c=video&a=setting&meunid='.$_GET['meunid']);
		}
	...
	public function add() {
		if ($_POST['dosubmit']) {
			...
			if (!$data['vid']) showmessage(L('failed_you_video_uploading'), 'goshouye.php?m=video&c=video&a=add');
			$data['title'] = isset($_POST['title']) && trim($_POST['title']) ? trim($_POST['title']) : showmessage(L('video_title_not_empty'), 'goshouye.php?m=video&c=video&a=add&meunid='.$_GET['meunid']);
			...
			if ($videoid) {
				showmessage(L('operation_success'), 'goshouye.php?m=video&c=video&a=init&meunid='.$_GET['meunid']);
			} else {
				showmessage(L('operation_failure'), 'goshouye.php?m=video&c=video&a=add&meunid='.$_GET['meunid']);
			}
		} else {
	 ...
	 * 视频手动添加方法
	 */
	public function add_diy() {
		if ($_POST['dosubmit']) {
			//首先处理，提交过来的数据
			$data['vid'] = safe_replace($_POST['vid']);
			if (!$data['vid']) showmessage(L('failed_you_video_uploading'), 'goshouye.php?m=video&c=video&a=add_diy');
			$data['title'] = isset($_POST['title']) && trim($_POST['title']) ? trim($_POST['title']) : showmessage(L('video_title_not_empty'), 'goshouye.php?m=video&c=video&a=add&meunid='.$_GET['meunid']);
			$data['description'] = trim($_POST['description']);
			$data['keywords'] = trim(strip_tags($_POST['keywords']));
			$data['picpath'] =  safe_replace($_POST['picpath']);
			$data['status'] =  21;
			$data['addtime'] = SYS_TIME;
			$data['userupload'] = intval($_POST['userupload']);
			$videoid = $this->v->add($data);
			if ($videoid) {
				...
				showmessage(L('operation_failure'), 'goshouye.php?m=video&c=video&a=add_diy&meunid='.$_GET['meunid']);
			}
		} else {

			include $this->admin_tpl('video_add_diy');
		}
	}
	/**
	 * function edit
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->ku6api->vms_edit($data)) {
				...
				if ($return) showmessage(L('operation_success'), 'goshouye.php?m=video&c=video&a=init');
				else showmessage(L('operation_failure'), 'goshouye.php?m=video&c=video&a=edit&vid='.$vid.'&menuid='.$_GET['menuid']);
			} else {
				showmessage($this->ku6api->error_msg, 'goshouye.php?m=video&c=video&a=edit&vid='.$vid.'&menuid='.$_GET['menuid']);
			}
		} else {
			$show_validator = true;
	...
	public function delete() {
		...
		if (!$this->ku6api->delete_v($r['vid'])) showmessage(L('operation_failure'), 'goshouye.php?m=video&c=video&a=init&meunid='.$_GET['meunid']);
		$this->v->del_video($vid);	
		showmessage(L('success_next_update_content'), 'goshouye.php?m=video&c=video&a=public_update_content&vid='.$vid.'&meunid='.$_GET['meunid']);
	}
	...
	public function public_update_content() {
		...
		if (!$result || empty($result)) {
			showmessage(L('update_complete'), 'goshouye.php?m=video&c=video&a=init&meunid='.$meunid);
		}
		//加载更新html类
		...
		showmessage(L('part_update_complete'), 'goshouye.php?m=video&c=video&a=public_update_content&vid='.$videoid.'&meunid='.$meunid);
	}
	...
	public function subscribe_list() {
		if (isset($_POST['dosubmit'])) {
			if (is_array($_POST['sub']) && !empty($_POST['sub'])) {
				...
				if ($result['code'] == 200) showmessage(L('operation_success'), 'goshouye.php?m=video&c=video&a=subscribe_list');
				else showmessage(L('subscribe_set_failed'), 'goshouye.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
			} else {
				showmessage(L('please_choose_catid_and_channel'), 'goshouye.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
			}
		} else {
			if(!$this->ku6api->testapi()) {
				showmessage(L('vms_sn_skey_error'),'?m=video&c=video&a=setting&menuid='.$_GET['menuid']);
	...
	public function sub_del() {
		$id = intval($_GET['id']);
		if (!$id) showmessage(L('illegal_parameters'), 'goshouye.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
		if ($this->ku6api->sub_del($id)) showmessage(L('operation_success'), 'goshouye.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
		else showmessage(L('delete_failed'), 'goshouye.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
	}
