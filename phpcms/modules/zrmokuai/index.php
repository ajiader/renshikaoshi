...
class index {
	...
	public function init()
	{
		...
		if ($_GET['mk']=='caikuai') {
			# 财会专题iframe调用模版
			include template('zrmokuai',  'caikuai');
		}else{
			# 默认模版
			include template('zrmokuai',  'index');
		}
    ...
    public function typelist()
    {
		...
		ob_start();
		include template('zrmokuai',  'typelist');
		echo mem_page(1);
	...
	*/
	...
	public function show()
	{
		...
		$id = intval($_GET['id']);
		if(!$id && !$typearr) showmessage('参数错误！','blank');
        
		$r = $this->db->get_one(array('id'=>$id));
		 
		ob_start();
		include template('zrmokuai',  'show');
		echo mem_page(1);
	}
