...
<tr><td colspan="2">
      ...
      <tr>
        ...
        <td>
          <?php echo menu_linkage(3362,'typeid',$vo['typeid']) ?></td>
      </tr>
      <tr>
        <th>模块名称：</th>
...
$(document).ready(function(){
	...
	$('#name').formValidator({onshow:"请输入模块名称!",onfocus:"请输入模块名称!",oncorrect:"输入正确!"}).inputValidator({min:1,onerror:"请输入模块名称!"}).ajaxValidator({type:"get",url:"",data:"m=zrmokuai&c=zr_admin&a=public_check_title&id=<?php echo $_GET['id']?>",datatype:"html",cached:false,async:'true',success : function(data) {
        if( data == "1" )
		{
            return true;
		}
