...
define('ASK_TITLE', '专家在线问答');//问答标题
define('ASK_PATH',$siteurl.'/question/');//问答首页路径
define('ASK_QLIST', $siteurl.'/question/?a=qlist');
define('ASK_LIST', $siteurl.'/question/?a=qlist&cid=');//问答分类列表页路径
define('ASK_SHOW', $siteurl.'/goshouye.php?m=zsask&c=zsask&a=answer&qid=');//问答显示页路径
define('ASK_QSTN', $siteurl.'/question/?a=question&cid='); //提问页路径
define('ASK_ANSWER', $siteurl.'/question/?a=answer&qid='); //答案页路径
...
function get_qinfo($qid) {
	 ...
	 * $catid 
	 */
	function get_question($catid = 0, $s = '') {
		$db = pc_base::load_model('ask_question_model');
		$where = get_catids($catid);
		$siteid = get_siteid();
		$where .= ' AND status !=1 and siteid='.$siteid;
		if($s){ 
			$where .= ' AND aid =0';
		 }else{
		    $where .= ' AND aid >0';
	...
	function getAnswerContent($aid) {
		$db = pc_base::load_model('ask_answer_model');
		$sql = 'aid='.$aid;
		$content = $db->get_one($sql,'content');
...
function get_category($siteid,$parentid = 0, $grade = 2, $style = 0, $format = '├─ ', $icon = '│ ', $iconz = '└─ ', $nbsp = '') {
	if ($style) $nbsp = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	$db = pc_base::load_model('ask_category_model');
	$where = array('parentid' =>$parentid,'siteid' => $siteid);
...
function ip2star($ip)
{
 ...
 //获取问答内容页面的伪静态规则
	function geturlrule($qid)
	{
	    $settings = getcache('settings', 'zsask');
		$rolecache = getcache('urlrules', 'commons');
		$roleurl =  $rolecache[$settings['urlrule_question']];
		if($roleurl)
		{
		    return siteurl(get_siteid()).str_replace('{$id}',$qid,$roleurl);
		}
	}
/**
	 * 问答排行榜标签
	 * @param $data
	 */
	 function zsak_hits($data) {
		$catid = intval($data['catid']);
		$siteid = intval($data['siteid']);

		$categorys = getcache('categorys_'.$siteid, 'zsask');
		...
		$hits = array();
		$result = $hits_db->select($sql, '*', $data['limit'], $order);
		...
		if($ids) {
			$sql = "aid>0 AND qid IN ($ids)";
		} else {
			$sql = '';
		}

		$question = pc_base::load_model('ask_question_model');
		$result =$question->select($sql, '*', $data['limit'],'','','qid');

		foreach ($ids_array as $id) {
			if($result[$id]['question']!='') {
				$array[$id] = $result[$id];
...
function gettypeurl()
{
	...
	$list = $type_db->select("module='teacher'");
	foreach ($list as $key => $value) {
		$arr[$value['typeid']] = $value;
	}
	return($arr);
}
?>