...
class index {
	...
	function __construct() {
		...
		//$this->admin = session_code('', $this->siteid);
		$this->config = getcache('settings', 'zsask');
		$this->IP = ip();
		$this->askcategorys = getcache('categorys_'.$this->siteid, 'zsask');
	...
	function init() 
	{
		...
		$SEO = seo($this->siteid,'',ASK_TITLE);
        $askcategorys = $this->askcategorys;
	...
	function qlist() {
		$siteid = $this->siteid;define('SITEID', $siteid);
		define('SITEID', $siteid);
		$askcategorys = getcache('categorys_'.$this->siteid, 'zsask');
		$caturl = siteurl($this->siteid).'goshouye.php?m=zsask&a=qlist&cid=';
		$catid = intval($_GET['cid']) ? intval($_GET['cid']) : 0;
		$s = intval($_GET['s']) ? intval($_GET['s']) : 0;
		if($s){ $ss='&s=1'; $n='n'; }
		$seotitle = ($s)?'待回答问题列表':'已回答问题列表-'.$askcategorys[$catid]['catname'];
		$SEO = seo($this->siteid,'',$seotitle);
		$page = max(intval($_GET['page']), 1);


		//查询分类
		if($catid){ $whereCatid = "and catid=$catid" ;}
		$where_answer = ($s>0)?"and aid = 0":"and aid > 0";
		// if($s>0){
  //           ="aid = 0";
  //       }else{
  //          $where_answer=;
  //       }
		$q  = safe_replace(trim(strip_tags($_GET['serch'])));
		if ($q == '请输入您的问题') {
			showmessage('请输入您的问题', ASK_PATH);
		}

        if($q){

		   $where_q = " and question like '%$q%'";
		}
        $pagesize =  10;
        $page = intval($_GET['page']);
		$page = max($page,1);

		$urlrule = ($catid>0) ? '/question/'.$n.'qlist_{$catid}_{$page}.html' : '/question/'.$n.'qlist_{$page}.html' ;

		$qlist = $this->question->listinfo("siteid=$this->siteid $whereCatid  $where_answer $where_q",'qid DESC',$page,$pagesize,'',9,$urlrule,array('catid'=>$catid));

		if (empty($qlist)) {
			#分词开始
			$http = pc_base::load_sys_class('http'); //加载http类
			$posturl = siteurl($this->siteid).'/kapi.php?op=get_keywords&number=3';
			...
			if($http->is_ok()) {
				...
				foreach ($fenciarr as $key => $value) {
					$value = trim($value);
					if (strlen($value) > 3) {
						$where_q .= "question like '%$value%' and ";
					}
				...
				if (strlen($where_q)>6) {
					...
					$where_q .= ')';
					$qlist = $this->question->listinfo("siteid=$this->siteid $whereCatid  $where_answer $where_q",'qid DESC',$page,$pagesize);
				}
	...
	function question() {
		...
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			//获取伪静态url 更新url
			$url = geturlrule($id);
			$this->question->update(array('url' => $url), array('qid'=>$id));
			if ($id) $this->hits->insert(array('hitsid'=>'zsask-'.$id, 'catid'=>$insert['catid']));
			//showmessage('操作成功！', HTTP_REFERER);
			showmessage('操作成功！', ASK_PATH);
			
		}
		$catid = intval($_GET['cid']) ? intval($_GET['cid']) : '';
		$SEO = seo($this->siteid,'','我要提问-'.ASK_TITLE);
	...
	function answer() {
		...
		define('SITEID', $siteid);
		$askcategorys = getcache('categorys_'.$this->siteid, 'zsask');
		$qid = intval($_GET['qid']) ? intval($_GET['qid']) : (intval($_POST['qid']) ? intval($_POST['qid']) : page404());
		if (!$qid)  page404();
		
		$info = $this->question->get_one('qid='.$qid.' AND status!=1 and siteid='.$siteid);
        if (!$info) showmessage('错误，问题不存在！');
		if ($info['status'] == 1) showmessage('问题正在审核或已删除！');
		...
		$SEO = seo($this->siteid,'', $info['question']);
		$page = max(intval($_GET['page']), 1);
		ob_start();
