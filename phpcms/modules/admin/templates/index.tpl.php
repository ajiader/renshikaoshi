...
<div id="content">
    ...
    <div class="crumbs">
    <div class="shortcut cu-span"><a href="?m=content&c=create_html&a=public_index&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('create_index')?></span></a>
    <a href="?m=admin&c=memcache_all&a=init&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span>Memcache页面缓存管理</span></a>
    <a href="?m=admin&c=cache_all&a=init&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('update_backup')?></span></a><a href="javascript:art.dialog({id:'map',iframe:'?m=admin&c=index&a=public_map', title:'<?php echo L('background_map')?>', width:'700', height:'500', lock:true});void(0);"><span><?php echo L('background_map')?></span></a><?php echo runhook('admin_top_left_menu')?></div>    <?php echo L('current_position')?><span id="current_pos"></span></div>
    	<div class="col-1">
...
function site_select(id,name, domain, siteid) {
	...
	$('#site_search').attr('href', 'goshouye.php?m=search&siteid='+siteid);
}
//隐藏站点下拉。
var s = 0;
var h;
...
function show_help(targetUrl) {
	$("#help").slideUp("slow");
	var str = '';
	$.getJSON("http://v9.help.phpcms.cn/kapi.php?jsoncallback=?",{op:'help',targetUrl: targetUrl},
	function(data){
		if(data!=null) {
			$("#help").slideDown("slow");
...
function _Site_M(project) {
	...
	$.getJSON('goshouye.php', {m:'admin', c:'index', a:'public_set_model', 'site_model':project, 'time':Math.random()}, function (data){
