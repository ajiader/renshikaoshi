...
<option value="<?=$v['typeid']?>" <?php if($v['typeid'] == $typeid): ?>selected="selected"<?php endif; ?>><?=$v['name']?></option>
<? endforeach; ?>
</select>

      <input name="search" type="submit" value="查询" class="button" />
<input type="hidden" name="m" value="admin" /><input  type="hidden"  name="c"  value="position"  /><input  type="hidden"  name="a" value="init" />
</form>
...
?>   
	...
	<td width="15%" align="center"><?php echo $info['modelid'] ? $model[$info['modelid']]['name'] : L('posid_all')?></td>
    	<td width="15%" align="center"><?php echo $info['typeid'] ? $types[$info['typeid']]['name'] : '<font color="#FF0000">未分类</font>'?></td>
