...
class plugin extends admin {
	...
	public function appcenter() {
		...
		$data = file_get_contents('http://open.phpcms.cn/goshouye.php?m=open&c=api&a=get_applist&s='.$s.'&p='.$p);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		
		$recommed_data = file_get_contents('http://open.phpcms.cn/goshouye.php?m=open&c=api&a=get_recommed_applist&s=5&p=1');
		$recommed_data = array_iconv(json_decode($recommed_data, true),'utf-8',CHARSET);
		
		$focus_data = file_get_contents('http://open.phpcms.cn/goshouye.php?m=open&c=api&a=get_app_focus&num=3');
	...
	public function appcenter_detail() {
		$data = array();
		$id = intval($_GET['id']);
		$data = file_get_contents('http://open.phpcms.cn/goshouye.php?m=open&c=api&a=get_detail_byappid&id='.$id);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		extract($data);		
		if($appname) {
...
EOF;
					...
					if($installres*$uninstallres*$cfgres > 0) {
						showmessage(L('plugin_configure_success', '', 'plugin'), 'goshouye.php?m=admin&c=plugin&a=import&dir='.$data['appenname']);
					} else {
			...
			if(empty($upgradezip_url)) {
				showmessage(L('download_fail', '', 'plugin'), 'goshouye.php?m=admin&c=plugin&a=appcenter');
			}
			...
			if($this->copyfailnum > 0) {
				...
				showmessage(L('download_success', '', 'plugin'), 'goshouye.php?m=admin&c=plugin&a=import&dir='.$appdirname);	
			}
		}
	}
