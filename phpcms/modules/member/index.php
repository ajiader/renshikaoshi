...
class index extends foreground {
	...
	public function register() {
		...
		if(!$member_setting['allowregister']) {
			showmessage(L('deny_register'), 'goshouye.php?m=member&c=index&a=login');
		}
		//加载短信模块配置
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				showmessage(L('operation_success'), 'goshouye.php?m=member&c=index&a=register&t=3');
			} else {
				//查看当前模型是否开启了短信验证功能
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if($status > 0) {
					...
					if($member_setting['enablemailcheck']) {
						...
						$url = APP_PATH."goshouye.php?m=member&c=index&a=register&code=$code&verify=1";
						...
						showmessage(L('operation_success'), 'goshouye.php?m=member&c=index&a=register&t=2');
					} else {
						//如果不需要邮箱认证、直接登录其他应用
						$synloginstr = $this->client->ps_member_synlogin($userinfo['phpssouid']);
						showmessage(L('operation_success').$synloginstr, 'goshouye.php?m=member&c=index&a=init');
					}
					
				}
			} else {
				showmessage(L('enable_register').L('enable_phpsso'), 'goshouye.php?m=member&c=index&a=login');
			}
			...
			if(!empty($_GET['verify'])) {
				$code = isset($_GET['code']) ? trim($_GET['code']) : showmessage(L('operation_failure'), 'goshouye.php?m=member&c=index');
				$phpcms_auth_key = md5(pc_base::load_config('system', 'auth_key'));
				$code_res = sys_auth($code, 'DECODE', $phpcms_auth_key);
				$code_arr = explode('|', $code_res);
				$userid = isset($code_arr[0]) ? $code_arr[0] : '';
				$userid = is_numeric($userid) ? $userid : showmessage(L('operation_failure'), 'goshouye.php?m=member&c=index');

				$this->db->update(array('groupid'=>$this->_get_usergroup_bypoint()), array('userid'=>$userid));
				showmessage(L('operation_success'), 'goshouye.php?m=member&c=index');
	...
	public function account_manage_avatar() {
		...
		$upurl = base64_encode($phpsso_api_url.'/goshouye.php?m=phpsso&c=index&a=uploadavatar&auth_data='.$auth_data);
		//获取头像数组
		$avatar = $this->client->ps_getavatar($this->memberinfo['phpssouid']);
	...
	public function account_manage_upgrade() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($memberinfo['amount'] >= $cost) {
				...
				showmessage(L('operation_success'), 'goshouye.php?m=member&c=index&a=init');
			} else {
				showmessage(L('operation_failure'), HTTP_REFERER);
			}
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if(isset($memberinfo['uid'])) {
					...
					if($status == -1) {	//用户不存在
						showmessage(L('user_not_exist'), 'goshouye.php?m=member&c=index&a=login');
					} elseif($status == -2) { //密码错误
						showmessage(L('password_error'), 'goshouye.php?m=member&c=index&a=login');
					} else {
						showmessage(L('login_failure'), 'goshouye.php?m=member&c=index&a=login');
					}
				}
				...
				if(!$r) showmessage(L('user_not_exist'),'goshouye.php?m=member&c=index&a=login');
				...
				if($r['password'] != $password) {				
					...
					showmessage(L('password_error', array('times'=>$times)), 'goshouye.php?m=member&c=index&a=login', 3000);
				}
			...
			param::set_cookie('_nickname', $nickname, $cookietime);
			//param::set_cookie('cookietime', $_cookietime, $cookietime);
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'goshouye.php?m=member&c=index';
	...
	public function logout() {
		...
		if($setting['snda_enable'] && param::get_cookie('_from')=='snda') {
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? urlencode($_GET['forward']) : '';
			$logouturl = 'https://cas.sdo.com/cas/logout?url='.urlencode(APP_PATH.'goshouye.php?m=member&c=index&a=logout&forward='.$forward);
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? $_GET['forward'] : 'goshouye.php?m=member&c=index&a=login';
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			if(!empty($me['id'])) {
				...
				if(!empty($r)) {
					...
					param::set_cookie('cookietime', $_cookietime, $cookietime);
					param::set_cookie('_nickname', $nickname, $cookietime);
					$forward = isset($_GET['forward']) && !empty($_GET['forward']) ? $_GET['forward'] : 'goshouye.php?m=member&c=index';
					showmessage(L('login_success').$synloginstr, $forward);
			...
			$aurl = $o->getAuthorizeURL($keys['oauth_token'] ,false , APP_PATH.'goshouye.php?m=member&c=index&a=public_sina_login&callback=1');
			$_SESSION['keys'] = $keys;
	...
	public function public_snda_login() {
		...
		define('SNDA_CALLBACK', urlencode(APP_PATH.'goshouye.php?m=member&c=index&a=public_snda_login&callback=1'));
	...
	public function public_qq_login() {
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			$oauth_callback = APP_PATH.'goshouye.php?m=member&c=index&a=public_qq_login&callback=1';
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$url = APP_PATH."goshouye.php?m=member&c=index&a=public_forget_password&code=$code";
			...
			showmessage(L('operation_success'), 'goshouye.php?m=member&c=index&a=login');
		} elseif($_GET['code']) {
			$phpcms_auth_key = md5(pc_base::load_config('system', 'auth_key').$this->http_user_agent);
			...
			if(is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
				...
				if(empty($memberinfo['phpssouid'])) {
					showmessage(L('operation_failure'), 'goshouye.php?m=member&c=index&a=login');
				}
				$updateinfo = array();
				$password = random(8);
