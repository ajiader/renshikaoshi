...
class content extends foreground {
	...
	public function publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('points_less_than',array('point'=>$memberinfo['point'],'need_point'=>abs($setting['presentpoint']))),APP_PATH.'goshouye.php?m=pay&c=deposit&a=pay&exchange=point',3000);
			...
			//缓存结果 END
			if($info['status']==99) {
				showmessage(L('contributors_success'), APP_PATH.'goshouye.php?m=member&c=content&a=published');
			} else {
				showmessage(L('contributors_checked'), APP_PATH.'goshouye.php?m=member&c=content&a=published');
			}
			...
			if (!$catid) showmessage(L('category').L('publish_deny'), APP_PATH.'goshouye.php?m=member');

			//判断本栏目是否允许投稿
			if (!$priv_db->get_one(array('catid'=>$catid, 'roleid'=>$memberinfo['groupid'], 'is_admin'=>0, 'action'=>'add'))) showmessage(L('category').L('publish_deny'), APP_PATH.'goshouye.php?m=member');
			$category = $CATEGORYS[$catid];
	...
	public function published() {
		...
		foreach($infos as $_v) {
			...
			$_v['url'] = $_v['status']==99 ? go($_v['catid'],$_v['id']) : APP_PATH.'goshouye.php?m=content&c=index&a=show&catid='.$_v['catid'].'&id='.$_v['id'];
	...
	public function info_publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			//缓存结果 END
			if($info['status']==99) {
				showmessage(L('contributors_success'), APP_PATH.'goshouye.php?m=member&c=content&a=info_top&id='.$id.'&catid='.$catid.'&msg=1');
			} else {
				showmessage(L('contributors_checked'), APP_PATH.'goshouye.php?m=member&c=content&a=info_top&id='.$id.'&catid='.$catid.'&msg=1');
			}
			...
			*/
			...
			if($zone == 0 && !isset($_GET['catid'])) {
				...
				$step = 2;	
				$step_1 = '<a href="'.APP_PATH.'goshouye.php?m=member&c=content&a=info_publish&siteid='.$siteid.'&city='.$citypinyin.'">'.$category['catname'].'</a>';
				include template('member', 'info_content_publish_select');
				exit;
			} elseif($zone == 0 && isset($_GET['catid'])) {
				$step = 3;	
				$step_1 = '<a href="'.APP_PATH.'goshouye.php?m=member&c=content&a=info_publish&siteid='.$siteid.'">'.$CATEGORYS[$category['parentid']]['catname'].'</a>';
				$step_2 = '<a href="'.APP_PATH.'goshouye.php?m=member&c=content&a=info_publish&siteid='.$siteid.'&city='.$citypinyin.'&catid='.$category['parentid'].'">'.$category['catname'].'</a>';			
				...
				$step = 4;				
				$step_1 = '<a href="'.APP_PATH.'goshouye.php?m=member&c=content&a=info_publish&siteid='.$siteid.'&city='.$citypinyin.'">'.$CATEGORYS[$category['parentid']]['catname'].'</a>';	
				$step_2 = '<a href="'.APP_PATH.'goshouye.php?m=member&c=content&a=info_publish&siteid='.$siteid.'&city='.$citypinyin.'&catid='.$category['parentid'].'">'.$category['catname'].'</a>';	
				$step_3 = '<a href="'.APP_PATH.'goshouye.php?m=member&c=content&a=info_publish&siteid='.$siteid.'&city='.$citypinyin.'&catid='.$catid.'">'.$zone_name.'</a>';	
	...
	public function upload_video() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if (is_array($categorys) && !empty($categorys)) {
				...
				if (empty($categorys)) showmessage(L('category').L('publish_deny'), APP_PATH.'goshouye.php?m=member');
				foreach ($categorys as $cid => $c) {
					if ($c['child']) {
						$ischild = 1;
