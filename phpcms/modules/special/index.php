...
class index {
	...
	public function type() {
		...
		extract($info);
        $special['title'] =    $info['title'];
		$css = get_css(unserialize($css));
		if(!$typeid) showmessage(L('illegal_action'));
		$type_db = pc_base::load_model('type_model');
		$types = $type_db->select(array('module'=>'special', 'parentid'=>$specialid), '*', '', '`listorder` ASC, `typeid` ASC', '', 'listorder');//所有分类
		$info = $type_db->get_one(array('typeid'=>$typeid));
       ;
		$SEO = seo($siteid, '', $info['name'].'-'.$special['title'], '');   ;
	...
	public function comment() {
		...
		$userid = param::get_cookie('_userid');
		if (!$userid) {
			showmessage(L('login_website'), APP_PATH.'goshouye.php?m=member&c=index');
		}
