...
class special_api {
			...
			if ($a == 'add' && !$v['del']) {
				...
				if ($siteid>1) {
					...
					if ($special_info['ishtml']) {
						...
						$url = $site_info['domain'].'goshouye.php?m=special&c=index&a=type&specialid='.$specialid.'&typeid='.$typeid;
					}
				} else {
					...
					else $url = APP_PATH.'goshouye.php?m=special&c=index&a=type&specialid='.$specialid.'&typeid='.$typeid;
				}
				...
				if ((!isset($v['del']) || empty($v['del'])) && $v['typeid']) {
					...
					if ($siteid>1) {
						...
						if ($special_info['ishtml']) {
							...
							$url = $site_info['domain'].'goshouye.php?m=special&c=index&a=type&specialid='.$specialid.'&typeid='.$v['typeid'];
						}
					} else {
						...
						//else $url = APP_PATH.'goshouye.php?m=special&c=index&a=type&specialid='.$specialid.'&typeid='.$v['typeid'];
						else $url = APP_PATH.'specials'.'/'.$v['typedir'].'/';
					}
					$v['url'] = $url;
					$typeid = $v['typeid'];
					unset($v['typeid']);
	...
	public function importfalbum($info = array()) {
		...
		if (is_array($info)) {
			...
			$url = $sitelists[$siteid]['domain'].'goshouye.php?m=special&c=index&id='.$specialid;
			...
			$url = $sitelists[$siteid]['domain'].'goshouye.php?m=special&c=index&a=type&specialid='.$specialid.'&typeid='.$typeid;
			$this->type_db->update(array('url'=>$url), array('typeid'=>$typeid));
			return $specialid;
		} else {
			return false;
