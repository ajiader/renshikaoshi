			...
			if ($id) {
				...
				if ($special['siteid']>1) {
					...
					if ($special['ishtml']) {
						...
						$url = $site_info['domain'].'goshouye.php?m=special&c=index&id='.$id;
					}
				} else {
					$url = $special['ishtml'] ? APP_PATH.substr(pc_base::load_config('system', 'html_root'), 1).'/special/'.$special['filename'].'/' : APP_PATH.'goshouye.php?m=special&c=index&id='.$id;
				}
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($special['ishtml'] && $special['filename']) {
				...
				if ($siteid>1) {
					$special['url'] = $site_info['domain'].'goshouye.php?m=special&c=index&specialid='.$_GET['specialid'];
				} else {
					//$special['url'] = APP_PATH.'goshouye.php?m=special&c=index&specialid='.$_GET['specialid'];
					$special['url'] = APP_PATH.'specials/';
				}
			}
