...
class index {
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
			if(trim($_GET['q'])=='') {
				header('Location: '.APP_PATH.'goshouye.php?m=search');exit;
			}
			if($siteid == 1){
			   $type_id = '1';
			}else{
			...
			$typeid = empty($_GET['typeid']) ? $type_id : intval($_GET['typeid']);
			...
			if($setting['sphinxenable']) {
				...
				*/
				...
				$sql = "`siteid`= '$siteid'   $sql_time AND `data` like '%$q%'";
				$result = $this->db->listinfo($sql, 'searchid DESC', $page, 10);
				// print_r($result);
				//如果为空就进行 并且分词存在 进行分词查询
				if (empty($result) && !empty($segment_q)) {
					$sql = "`siteid`= '$siteid'  $sql_time AND MATCH (`data`) AGAINST ('$segment_q' IN BOOLEAN MODE)";
					$result = $this->db->listinfo($sql, 'searchid DESC', $page, 10);
				}
			}
	...
	public function public_get_suggest_keyword() {
		...
		$urm_md5 = md5($url);
		echo(!in_array($urm_md5, $trust_url));
