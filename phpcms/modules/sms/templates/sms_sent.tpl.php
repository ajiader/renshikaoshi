...
<form name="smsform" action="goshouye.php?m=sms&c=sms&a=exportmobile" method="post" >
...
function select_tpl(obj) {
	...
	$.get("goshouye.php", {m:'sms', c:'sms', a:'public_get_tpl', sceneid:sceneid, tm:Math.random()}, function (data) {
		if (data) {
			$('#tpl').html(data);
		} else {
...
function show_tpl(id) {
	...
	$.get("goshouye.php", {m:'sms', c:'sms', a:'public_show_tpl', tplid:tplid, tm:Math.random()}, function (data) {
		if (data) {
			$('#tpl_case').html(data);
		} else {
