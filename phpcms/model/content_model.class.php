...
class content_model extends model {
	...
	public function add_content($data,$isimport = 0) {
		//$catid_caiji = array('281','282','283','284','307');
		//采集的要审核
		if(in_array($data['catid'],$catid_caiji)) {
					    $data['status'] = 1;
		}
		...
		//更新到全站搜索
		if(!in_array($data['catid'],$catid_caiji)) { //采集的文章不更新
	...
	public function edit_content($data,$id) {
		...
		//删除当前文章memcache缓存
		delcache(str_replace('http://','',$systeminfo['url']),'','memcache','template');
	   ...
	   $hitsidarr = $this->hits_db->get_one(array('hitsid'=>$hitsid));
	   if(empty($hitsidarr)){
	       $this->hits_db->insert(array('hitsid'=>$hitsid,'catid'=>$systeminfo['catid'],'updatetime'=>SYS_TIME));
       }
	...
	public function status($ids = array(), $status = 99) {
		...
		if(is_array($ids) && !empty($ids)) {
			foreach($ids as $id) {
				...
				if($status==0) {
				//退稿发送短消息、邮件
					$message = L('reject_message_tips').$r['title']."<BR><a href=\'goshouye.php?m=member&c=content&a=edit&catid={$r[catid]}&id={$r[id]}\'><font color=red>".L('click_edit')."</font></a><br>";
